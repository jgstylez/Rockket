...
const LaunchPads: React.FC<LaunchPadsProps> = ({ onDeploy, templates, stage }) => {
  ...
  const startDeployment = () => {
    setIsDeploying(true);
    setDeploymentLog(['> INITIALIZING_DEPLOYMENT_SEQUENCE...']);
  };
  ...
  useEffect(() => {
    if (isDeploying && selectedPad) {
      const sequence = [
        { msg: `> TARGET: ${selectedPad.name.toUpperCase()}_CHASSIS`, delay: 500 },
        { msg: '> VERIFYING_USER_CREDENTIALS... [OK]', delay: 1000 },
        { msg: '> ALLOCATING_CLOUD_RESOURCES...', delay: 1500 },
        { msg: '> PROVISIONING_DATABASE_CLUSTER...', delay: 2200 },
        { msg: `> INJECTING_CONFIG: ${selectedPad.features.join('_MODULE, ')}_MODULE`, delay: 3000 },
        { msg: '> ESTABLISHING_SECURE_UPLINK...', delay: 3800 },
        { msg: '> DEPLOYING_FRONTEND_CONTAINERS...', delay: 4200 },
        { msg: '> SYSTEM_CALIBRATION_COMPLETE.', delay: 4800 },
        { msg: '> READY_FOR_IGNITION.', delay: 5000 },
      ];
  ...
  const getIcon = (name: string) => {
    switch(name) {
      ...
      case 'Genesis Blueprint': return <PenTool size={32} className="text-white" />;
  ...
  if (isDeploying) {
    return (
      <div className="fixed inset-0 z-50 bg-black font-mono flex flex-col items-center justify-center p-8 animate-fade-in">
        {/* Background Grid */}
        ...
        <div className="relative z-10 w-full max-w-3xl border border-emerald-500/30 bg-black/80 backdrop-blur-md rounded-lg overflow-hidden shadow-[0_0_50px_rgba(16,185,129,0.2)]">
          ...
          <div className="bg-emerald-900/20 border-b border-emerald-500/30 p-4 flex items-center justify-between">
             <div className="flex items-center gap-2">
               ...
               <span className="text-emerald-500 font-bold text-sm tracking-widest">DEPLOYMENT_CONSOLE_V2.4</span>
             </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-end border-b border-slate-200 dark:border-slate-800 pb-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 max-w-xl">
            {stage === 'ideation' 
              ? 'Select a blueprint to begin structuring your mission concept.'
              : 'Select a pre-configured industry chassis to auto-provision your infrastructure.'}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 p-4">
        {displayedTemplates.map((template) => (
          ...
          >
            ...
            <GlassCard className={`relative h-full flex flex-col transform transition-transform duration-500 group-hover:-translate-y-2 group-hover:scale-[1.02] border-slate-200 dark:border-slate-700 ${
        ...
        <div className="fixed bottom-0 left-0 right-0 z-40 p-6 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-t border-slate-200 dark:border-slate-800 animate-fade-in-up">
           <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white">Confirm Deployment</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400">
                       Initialize <span className="font-bold text-indigo-500">{selectedPad.name}</span> environment?
                    </p>
                 </div>
              </div>
