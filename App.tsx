...
const App: React.FC = () => {
  ...
  const handleOnboardingComplete = (data: { stage: Stage }) => {
    setStage(data.stage);
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'mission_cockpit': return <MissionCockpit />;
  ...
  return (
    <div className={`min-h-screen bg-space-50 dark:bg-space-950 text-slate-900 dark:text-slate-200 font-sans selection:bg-orange-500/30 transition-colors duration-300 ${!isOnboardingComplete ? 'overflow-hidden' : ''}`}>
      ...
      >
        ...
        >
          ...
          <Activity size={10} className="mr-1 text-emerald-500 animate-pulse" /> DEV CONSOLE
        </div>
        <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border border-slate-200 dark:border-slate-700 p-2 rounded-xl shadow-2xl flex items-center gap-3">
          <div className="flex bg-slate-100 dark:bg-slate-800 rounded-lg p-1">
            <button onClick={() => handleStageSwitch('ideation')} className={`px-3 py-1.5 text-[10px] font-bold uppercase rounded-md transition-all ${stage === 'ideation' ? 'bg-indigo-500 text-white shadow' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}>Genesis</button>
            <button onClick={() => handleStageSwitch('setup')} className={`px-3 py-1.5 text-[10px] font-bold uppercase rounded-md transition-all ${stage === 'setup' ? 'bg-orange-500 text-white shadow' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}>Ignition</button>
            <button onClick={() => handleStageSwitch('growth')} className={`px-3 py-1.5 text-[10px] font-bold uppercase rounded-md transition-all ${stage === 'growth' ? 'bg-emerald-500 text-white shadow' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}>Velocity</button>
          </div>
          ...
          >
            ...
            <span className={`text-[10px] font-bold ${currentView === 'admin' ? 'block' : 'hidden md:block'}`}>
              {currentView === 'admin' ? 'GOD MODE ON' : 'GOD MODE'}
            </span>
          </button>
        </div>
      </div>
              ...
              <div className="mt-8 px-2 lg:px-4 space-y-8">
                ...
                <div>
                  ...
                  <nav className="space-y-1">
                    {[
                      { id: 'dashboard', label: 'The Bridge', icon: LayoutDashboard },
            ...
            <header className="h-20 border-b border-slate-200 dark:border-slate-800/50 bg-white/60 dark:bg-slate-900/40 backdrop-blur-md flex items-center justify-between px-8 transition-colors duration-300 z-10">
              ...
              <div className="flex items-center">
                <div className="flex items-center text-slate-500 dark:text-slate-400 text-sm font-mono mr-6 border-r border-slate-300 dark:border-slate-700 pr-6 h-8">
                  ...
                  <span className="opacity-50">SYSTEMS ONLINE //</span>
                  <span className="ml-2 text-emerald-600 dark:text-emerald-500 hidden md:inline">SECURE CONNECTION</span>
                </div>
